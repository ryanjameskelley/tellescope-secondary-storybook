...
import { ChevronDown, CirclePlus, CheckCheck, Pencil, AlarmClockMinus, Trash, CircleX, Plus, Check, X, Circle, PlusCircle, Settings2 } from "lucide-react"
import { SortableHeader, StaticHeader } from "@/components/atoms/table"
import { StatusFilter, type StatusFilterItem, TableSearch, TableHeaderSection, type HeaderColumn, TableActions, TableControls, TablePagination, RowSelector, DraggableRowHandle, TagSelector, TagList, type TagSelectorOption, type TagListProps, type TagColor } from "@/components/molecules/table"
...
export function TicketingTable() {
  ...
  const handleStatusFilterChange = (selectedStatuses: string[]) => {
    setSelectedStatusFilters(selectedStatuses)
  }
    ...
    <div className="w-full">
      <div className="flex items-center justify-between py-4">
        <TableSearch
          ...
          showFilter={true}
          filterLabel="Filter"
          filterItems={statusFilterItems}
          selectedFilters={selectedStatusFilters}
          onFilterSelectionChange={handleStatusFilterChange}
            ...
            <Table>
              <TableHeaderSection 
                columns={visibleScrollableColumns.map((column) => ({
                  key: column.key,
                  label: column.label,
                  sortable: column.key === 'opened' || column.key === 'origin',
                  onSort: (column.key === 'opened' || column.key === 'origin') 
                    ? () => table.getColumn(column.key)?.toggleSorting(table.getColumn(column.key)?.getIsSorted() === 'asc')
                    : undefined,
